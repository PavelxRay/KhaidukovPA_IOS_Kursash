        ...
        VStack {
            ScrollView {
                VStack(spacing: 24) {
                    ...
                    // Кнопки управления скачиванием / чтением
                    if isDownloaded {
                        Button(action: {
                            isReadyToRead = true
                        }) {
            ...
            // Навигационный мост вынесен в корень body для правильной работы в иерархии View
            NavigationLink(
                destination: ReaderView(
                    bookTitle: book.title,
                    fileURL: StorageManager.shared.getLocalFileURL(for: book.id),
                    bookId: book.id // <-- ДОБАВЛЯЕМ ЭТУ СТРОКУ
                ),
